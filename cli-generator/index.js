...
const run = async () => {
  ...
  // Clone MFE template repo
  const repo = 'https://github.com/jamayoral/mfe-template'
  ...
  clone(repo, targetPath)
    .then(() => {
      ...
      shell.sed('-i', 'template', name, `${targetPath}/package.json`)
      shell.sed(
        '-i',
        'template',
        name,
